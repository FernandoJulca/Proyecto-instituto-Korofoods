import {
  Component,
  EventEmitter,
  inject,
  OnDestroy,
  OnInit,
  Output,
} from '@angular/core';
import { CommonModule } from '@angular/common';
import { FormsModule } from '@angular/forms';
import { Subscription } from 'rxjs';
import { HistorialUsuario } from '../../../shared/response/historialUsuarioResponse.model';
import { ChatService } from '../../service/chat.service';
import { WebsocketService } from '../../service/websocket.service';
import { UserService } from '../../service/user.service';
import { Recepcionista } from '../../../shared/response/recepcionistaResponse.model';

@Component({
  selector: 'app-chat-list',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './chat-list.component.html',
  styleUrl: './chat-list.component.css',
})
export class ChatListComponent implements OnInit, OnDestroy {
  @Output() chatSelected = new EventEmitter<HistorialUsuario>();

  private chatService = inject(ChatService);
  private wsService = inject(WebsocketService);
  private userService = inject(UserService);

  chats: HistorialUsuario[] = [];
  recepcionistas: Recepcionista[] = [];
  filteredChats: HistorialUsuario[] = [];

  searchQuery: string = '';
  showNewChatModal: boolean = false;
  selectedRecepcionistaId: number | null = null;

  currentUserId: number = 0;
  selectedChatId: string | null = null;

  private notificationSubscription?: Subscription;

  ngOnInit(): void {
    this.loadCurrentUser();
    this.loadChats();
    this.subscribeToNotifications();
  }

  ngOnDestroy(): void {
    this.notificationSubscription?.unsubscribe();
  }

  private loadCurrentUser(): void {
    const user = this.userService.getUser();

    if (user) {
      this.currentUserId = user.idUsuario || 0;
    }
  }

  private loadChats(): void {
    if (!this.currentUserId) return;

    this.chatService.obtenerHistorialChats(this.currentUserId).subscribe({
      next: (chats) => {
        this.chats = chats;
        this.filteredChats = chats;
        console.log('Chats cargados: ', chats);
      },
      error: (error) => {
        console.error('Error al cargar chats:', error);
      },
    });
  }

  public refreshChats(): void {
    this.loadChats();
  }

  private subscribeToNotifications(): void {
    this.notificationSubscription = this.wsService.onNotification().subscribe({
      next: (mensaje) => {
        if (mensaje) {
          console.log('Nueva notificación:', mensaje);

          // Actualizar la lista de chats cuando llega un mensaje nuevo
          this.updateChatInList(mensaje.chatId, mensaje.contenido);
        }
      },
    });
  }

  private updateChatInList(chatId: string, ultimoMensaje: string): void {
    const chat = this.chats.find((c) => c.chatId === chatId);
    if (chat) {
      chat.ultimoMensaje = ultimoMensaje;
      chat.fechaUltimoMensaje = new Date();

      // Mover el chat al principio de la lista
      this.chats = [chat, ...this.chats.filter((c) => c.chatId !== chatId)];
      this.filteredChats = [...this.chats];
    } else {
      // Si es un chat nuevo, recargar la lista completa
      this.loadChats();
    }
  }

  onSearchChange(): void {
    const query = this.searchQuery.toLowerCase().trim();

    if (!query) {
      this.filteredChats = [...this.chats];
      return;
    }

    this.filteredChats = this.chats.filter((chat) => {
      const nombre = `${chat.nombre} ${chat.apePaterno}`.toLowerCase();
      const mensaje = chat.ultimoMensaje.toLowerCase();
      return nombre.includes(query) || mensaje.includes(query);
    });
  }

  selectChat(chat: HistorialUsuario): void {
    this.selectedChatId = chat.chatId;
    this.chatSelected.emit(chat);
  }

  openNewChatModal(): void {
    this.loadRecepcionistas();
    this.showNewChatModal = true;
    this.selectedRecepcionistaId = null;
  }

  closeNewChatModal(): void {
    this.showNewChatModal = false;
    this.selectedRecepcionistaId = null;
  }

  private loadRecepcionistas(): void {
    this.chatService.obtenerRecepcionistas().subscribe({
      next: (recepcionistas) => {
        this.recepcionistas = recepcionistas;
        console.log('Recepcionistas cargados:', recepcionistas);
      },
      error: (error) => {
        console.error('Error al cargar recepcionistas:', error);
      },
    });
  }

  selectRecepcionista(recepcionistaId: number): void {
    this.selectedRecepcionistaId = recepcionistaId;
  }

  startNewChat(): void {
    if (!this.selectedRecepcionistaId) {
      alert('Por favor selecciona un recepcionista');
      return;
    }

    this.chatService
      .iniciarChat(this.currentUserId, this.selectedRecepcionistaId)
      .subscribe({
        next: (chat) => {
          console.log('Chat iniciado:', chat);

          //Recargamos la lista
          this.loadChats();

          //Cerramos el modal
          this.closeNewChatModal();

          setTimeout(() => {
            const nuevoChat = this.chats.find((c) => c.chatId === chat.id);
            if (nuevoChat) {
              this.selectChat(nuevoChat);
            }
          }, 500);
        },

        error: (error) => {
          console.error('Error al iniciar chat:', error);
          alert('No se pudo iniciar el chat');
        },
      });
  }

  formatTime(date: Date): string {
    if (!date) return '';

    const messageDate = new Date(date);
    const today = new Date();
    const yesterday = new Date(today);
    yesterday.setDate(yesterday.getDate() - 1);

    // Mismo día
    if (messageDate.toDateString() === today.toDateString()) {
      return messageDate.toLocaleTimeString('es-PE', {
        hour: '2-digit',
        minute: '2-digit',
      });
    }

    // Ayer
    if (messageDate.toDateString() === yesterday.toDateString()) {
      return 'Ayer';
    }

    // Otra fecha
    return messageDate.toLocaleDateString('es-PE', {
      day: '2-digit',
      month: 'short',
    });
  }

  truncateMessage(message: string, maxLength: number = 35): string {
    if (!message) return '';
    return message.length > maxLength
      ? message.substring(0, maxLength) + '...'
      : message;
  }

  getAvatarUrl(imagen: string): string {
    return imagen || '/img/no-imagen.jpg';
  }
}
